...
<Frame caption="Use memory on, in Automatic mode, with Record to memory below">
  <img src="/images/en/whatsapp-agents/memory-group.png" alt="AI Memory group with the Use memory toggle on, the Automatic and Variables mode selector, Deep recall off, and the Record to memory toggle" />
</Frame>
...
In **Variables** mode the nine tokens appear as chips under the selector. Clicking one drops it
into the prompt at the cursor.

<Frame caption="Variables mode: the nine memory tokens, ready to drop into the prompt">
  <img src="/images/en/whatsapp-agents/memory-variables.png" alt="AI Memory group in Variables mode showing the nine memory chips from Memory to Memory last sentiment" />
</Frame>
...
<Frame caption="The four categories, each with its own instruction box">
  <img src="/images/en/whatsapp-agents/memory-modal.png" alt="What this agent records modal with the Facts, Preferences, Open items and Milestones categories ticked, and the business context field" />
</Frame>
...
<Frame caption="Further down the same modal: the hard exclusion and when memory is written">
  <img src="/images/en/whatsapp-agents/memory-modal-exclusions.png" alt="Never record field and the When to record selector with During the conversation and When it ends" />
</Frame>
